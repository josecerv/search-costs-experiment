...
```{r mod-black-by-urm-faculty, results='asis'}
# Run moderation analysis specifically for Black speakers by URM faculty percentage
mod_results_black <- run_moderation_by_demographic(
  demographic = "black",
  moderator_var = "frac_urm_faculty",
  moderator_label = "\\% URM Faculty",
  base_controls = base_controls,
  extended_controls = extended_controls,
  exclude_controls = c("frac_urm_faculty"),
  center = FALSE
)

# Add to all_models_list if results exist
if (!is.null(mod_results_black$model_info_list)) {
  all_models_list <- append(all_models_list, mod_results_black$model_info_list)
}
```
