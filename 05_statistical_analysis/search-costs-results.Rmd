...
```{r discipline-moderation, warning=FALSE, message=FALSE}
cat("\n\n\\clearpage\n")
cat("\\section{Exploratory Analysis: Discipline Moderation}\n\n")

# First check discipline distribution
cat("\\subsection{Distribution Across Disciplines}\n\n")

discipline_dist <- data %>%
  group_by(discipline) %>%
  summarise(
    n_seminars = n(),
    n_departments = n_distinct(department_std),
    ...
    pct_of_total = 100 * n() / nrow(data)
  ) %>%
  arrange(desc(n_seminars))

cat("\\begin{table}[H]\n")
cat("\\caption{Sample Distribution by Discipline}\n")
cat("\\centering\n")
cat("\\begin{tabular}{lrrrrr}\n")
cat("\\toprule\n")
cat("Discipline & Seminars & Departments & Treatment & Control & \\% of Total \\\\\n")
cat("\\midrule\n")

for (i in 1:nrow(discipline_dist)) {
  cat(sprintf("%s & %d & %d & %d & %d & %.1f\\%% \\\\\n",
              discipline_dist$discipline[i],
              discipline_dist$n_seminars[i],
              discipline_dist$n_departments[i],
              discipline_dist$n_treatment[i],
              discipline_dist$n_control[i],
              discipline_dist$pct_of_total[i]))
}

cat("\\midrule\n")
cat(sprintf("Total & %d & %d & %d & %d & 100.0\\%% \\\\\n",
            sum(discipline_dist$n_seminars),
            length(unique(data$department_std)),
            sum(discipline_dist$n_treatment),
            sum(discipline_dist$n_control)))
cat("\\bottomrule\n")
cat("\\end{tabular}\n")
cat("\\end{table}\n\n")

# Full interaction model
cat("\\subsection{Full Interaction Model}\n\n")
cat("We estimate the following model where treatment effects vary by discipline:\n\n")
cat("$$Y_{it} = \\beta_0 + \\sum_{d} \\beta_{1d} \\cdot \\text{Treatment}_i \\times \\text{Discipline}_d + \\sum_{d} \\beta_{2d} \\cdot \\text{Discipline}_d + \\epsilon_{it}$$\n\n")

# Function to run discipline moderation
run_discipline_moderation <- function(outcome_var, outcome_label) {
  cat(sprintf("\\subsubsection{Outcome: %s}\n\n", outcome_label))
  
  # Create discipline dummies (using Computer Science as reference)
  model_data <- data %>%
    mutate(
      is_physics = as.numeric(discipline == "Physics"),
      is_math = as.numeric(discipline == "Mathematics"),
      is_chemistry = as.numeric(discipline == "Chemistry"),
      is_mae = as.numeric(discipline == "Mechanical Engineering"),
      is_cs = as.numeric(discipline == "Computer Science")
    )
  
  # Model 1: Main effects only
  formula1 <- as.formula(paste0(outcome_var, " ~ treatment + is_physics + is_math + is_chemistry + is_mae"))
  model1 <- lm(formula1, data = model_data)
  model1$cluster_se <- get_clustered_se(model1, model_data$department_std)
  
  # Model 2: Full interaction
  formula2 <- as.formula(paste0(outcome_var, " ~ treatment * (is_physics + is_math + is_chemistry + is_mae + is_cs) - treatment:is_cs"))
  model2 <- lm(formula2, data = model_data)
  model2$cluster_se <- get_clustered_se(model2, model_data$department_std)
  
  # Model 3: With controls
  formula3 <- as.formula(paste0(outcome_var, " ~ treatment * (is_physics + is_math + is_chemistry + is_mae + is_cs) - treatment:is_cs + ",
                               "dept_ranking + total_faculty + frac_urm_faculty"))
  model3 <- lm(formula3, data = model_data)
  model3$cluster_se <- get_clustered_se(model3, model_data$department_std)
  
  # Extract treatment effects by discipline
  cat("\\begin{table}[H]\n")
  cat(sprintf("\\caption{Treatment Effects by Discipline: %s}\n", outcome_label))
  cat("\\centering\n")
  cat("\\begin{tabular}{lccc}\n")
  cat("\\toprule\n")
  cat(" & (1) & (2) & (3) \\\\\n")
  cat(" & Pooled Effect & By Discipline & With Controls \\\\\n")
  cat("\\midrule\n")
  cat("\\textbf{Treatment Effects} & & & \\\\\n")
  
  # For Model 2 and 3, calculate discipline-specific effects
  disciplines <- c("Physics", "Mathematics", "Chemistry", "MAE", "Computer Science")
  disc_vars <- c("is_physics", "is_math", "is_chemistry", "is_mae", "is_cs")
  
  # Model 1: Just show overall treatment
  treatment_coef <- coef(model1)["treatment"]
  treatment_se <- model1$cluster_se["treatment"]
  treatment_t <- treatment_coef / treatment_se
  treatment_pval <- 2 * pt(-abs(treatment_t), df = df.residual(model1))
  
  cat(sprintf("Pooled & %.4f%s & & \\\\\n",
              treatment_coef,
              format_sig_stars(treatment_pval)))
  cat(sprintf(" & (%.4f) & & \\\\\n", treatment_se))
  
  cat(" & & & \\\\\n")
  
  # Models 2 and 3: Show discipline-specific effects
  for (i in 1:length(disciplines)) {
    cat(sprintf("%s & & ", disciplines[i]))
    
    # Model 2
    if (disciplines[i] == "Computer Science") {
      # Reference category
      effect2 <- coef(model2)["treatment"]
      se2 <- model2$cluster_se["treatment"]
      t2 <- effect2 / se2
      pval2 <- 2 * pt(-abs(t2), df = df.residual(model2))
    } else {
      # Other disciplines
      interaction_term <- paste0("treatment:", disc_vars[i])
      base_effect <- coef(model2)["treatment"]
      interaction_effect <- ifelse(interaction_term %in% names(coef(model2)),
                                  coef(model2)[interaction_term], 0)
      effect2 <- base_effect + interaction_effect
      
      # Calculate SE using delta method with clustered vcov
      vcov_cluster <- vcovCL(model2, cluster = model_data$department_std)
      var_effect <- vcov_cluster["treatment", "treatment"]
      if (interaction_term %in% rownames(vcov_cluster)) {
        var_effect <- var_effect + vcov_cluster[interaction_term, interaction_term] +
                     2 * vcov_cluster["treatment", interaction_term]
      }
      se2 <- sqrt(var_effect)
      pval2 <- 2 * (1 - pnorm(abs(effect2/se2)))
    }
    
    cat(sprintf("%.4f%s & ", effect2, format_sig_stars(pval2)))
    
    # Model 3
    if (disciplines[i] == "Computer Science") {
      effect3 <- coef(model3)["treatment"]
      se3 <- model3$cluster_se["treatment"]
      t3 <- effect3 / se3
      pval3 <- 2 * pt(-abs(t3), df = df.residual(model3))
    } else {
      interaction_term <- paste0("treatment:", disc_vars[i])
      base_effect <- coef(model3)["treatment"]
      interaction_effect <- ifelse(interaction_term %in% names(coef(model3)),
                                  coef(model3)[interaction_term], 0)
      effect3 <- base_effect + interaction_effect
      
      vcov_cluster <- vcovCL(model3, cluster = model_data$department_std)
      var_effect <- vcov_cluster["treatment", "treatment"]
      if (interaction_term %in% rownames(vcov_cluster)) {
        var_effect <- var_effect + vcov_cluster[interaction_term, interaction_term] +
                     2 * vcov_cluster["treatment", interaction_term]
      ...
      se3 <- sqrt(var_effect)
      pval3 <- 2 * (1 - pnorm(abs(effect3/se3)))
    }
    
    cat(sprintf("%.4f%s \\\\\n", effect3, format_sig_stars(pval3)))
    cat(sprintf(" & & (%.4f) & (%.4f) \\\\\n", se2, se3))
  }
  ...
  cat("\\bottomrule\n")
  cat("\\end{tabular}\n")
  cat("\\parbox{\\linewidth}{\\footnotesize Note: Clustered standard errors at department level in parentheses. Computer Science is the reference category. MAE = Mechanical Engineering. Controls include department ranking, total faculty, and fraction URM faculty. Significance: + p<0.1; * p<0.05; ** p<0.01; *** p<0.001.}\n")
  cat("\\end{table}\n\n")
  
  # Wald tests for pairwise comparisons
  cat("\\textbf{Wald Tests: Pairwise Comparisons of Treatment Effects}\n\n")
  
  # Use Model 3 (with controls) for tests
  cat("\\begin{table}[H]\n")
  cat("\\caption{Pairwise Comparisons of Discipline-Specific Treatment Effects}\n")
  cat("\\centering\n")
  cat("\\begin{tabular}{lcc}\n")
  cat("\\toprule\n")
  cat("Comparison & Difference (SE) & p-value \\\\\n")
  cat("\\midrule\n")
  
  # Get all pairwise comparisons
  for (i in 1:(length(disciplines)-1)) {
    for (j in (i+1):length(disciplines)) {
      disc1 <- disciplines[i]
      disc2 <- disciplines[j]
      
      # Calculate difference
      if (disc1 == "Computer Science") {
        diff <- -coef(model3)[paste0("treatment:", disc_vars[j])]
        # SE is just the SE of the interaction term
        se_diff <- model3$cluster_se[paste0("treatment:", disc_vars[j])]
      } else if (disc2 == "Computer Science") {
        diff <- coef(model3)[paste0("treatment:", disc_vars[i])]
        se_diff <- model3$cluster_se[paste0("treatment:", disc_vars[i])]
      } else {
        # Difference between two interaction terms
        term1 <- paste0("treatment:", disc_vars[i])
        term2 <- paste0("treatment:", disc_vars[j])
        
        coef1 <- ifelse(term1 %in% names(coef(model3)), coef(model3)[term1], 0)
        coef2 <- ifelse(term2 %in% names(coef(model3)), coef(model3)[term2], 0)
        diff <- coef1 - coef2
        
        # Calculate SE
        vcov_cluster <- vcovCL(model3, cluster = model_data$department_std)
        var_diff <- 0
        if (term1 %in% rownames(vcov_cluster)) {
          var_diff <- var_diff + vcov_cluster[term1, term1]
        }
        if (term2 %in% rownames(vcov_cluster)) {
          var_diff <- var_diff + vcov_cluster[term2, term2]
        }
        if (term1 %in% rownames(vcov_cluster) && term2 %in% rownames(vcov_cluster)) {
          var_diff <- var_diff - 2 * vcov_cluster[term1, term2]
        }
        se_diff <- sqrt(var_diff)
      }
      
      pval <- 2 * (1 - pnorm(abs(diff/se_diff)))
      
      cat(sprintf("%s - %s & %.4f (%.4f) & %.4f%s \\\\\n",
                  disc1, disc2, diff, se_diff, pval, format_sig_stars(pval)))
    }
  }
  
  cat("\\bottomrule\n")
  cat("\\end{tabular}\n")
  cat("\\parbox{\\linewidth}{\\footnotesize Note: Based on Model 3 with controls. Positive values indicate the first discipline has a larger treatment effect. Significance: + p<0.1; * p<0.05; ** p<0.01; *** p<0.001.}\n")
  cat("\\end{table}\n\n")
...
# Run analyses
black_disc_model <- run_discipline_moderation("pct_black", "Percentage Black Speakers")
any_black_disc_model <- run_discipline_moderation("has_any_black", "Any Black Speakers")

# Summary interpretation
cat("\\subsection{Summary of Exploratory Analyses}\n\n")

cat("\\textbf{Key Findings:}\n\n")
cat("\\begin{enumerate}\n")
cat("\\item \\textbf{Seniority Moderation:} We examined whether the treatment effect varies by speaker seniority (years since PhD). ")
cat("The continuous moderation analysis tests for a linear interaction between treatment and years since PhD, ")
cat("while the subgroup analysis compares effects for junior vs senior speakers (split at median).\n\n")

cat("\\item \\textbf{Discipline Moderation:} We tested whether treatment effects differ across the five academic disciplines ")
cat("in our sample. The full interaction model allows each discipline to have its own treatment effect, ")
cat("and pairwise Wald tests examine whether these differences are statistically significant.\n")
cat("\\end{enumerate}\n\n")

cat("\\textbf{Note:} These are exploratory analyses not pre-registered in our analysis plan. ")
cat("Results should be interpreted with appropriate caution regarding multiple testing.\n")