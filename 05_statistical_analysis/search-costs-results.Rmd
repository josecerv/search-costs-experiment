...
create_regression_table <- function(models, title, label, demographic_group = NULL, 
  ...
  # 1. Remove vertical space commands like \\[-1.8ex]
  stargazer_output <- gsub("\\\\\\[-+]?[0-9\\.]+ex\\]", "]", stargazer_output, perl = TRUE)
  
  # 2. Remove rows that are only ampersands and spaces
  ...
  # Fix tabular column specification for 6-column tables
  if (n_models == 6 && length(tabular_line_idx) > 0) {
    # Replace the \begin{tabular} line with better column specification
    tabular_line <- stargazer_output[tabular_line_idx]
    # Use fixed width columns for better alignment
    new_tabular <- "\\begin{tabular}{@{\\extracolsep{2pt}}l*{6}{c}}"
  ...
  if (!"Category" %in% names(all_significant_results)) {
    ...
    for (i in 1:nrow(all_significant_results)) {
      if (grepl("Main Effect|Main Question", all_significant_results$Analysis[i])) {
        ...
        all_significant_results$Category[i] <- "Demographic Analysis"
      } else if (grepl("Semester", all_significant_results$Analysis[i])) {
        all_significant_results$Category[i] <- "Semester Analysis"
      } else if (grepl("Discipline:", all_significant_results$Analysis[i])) {
        all_significant_results$Category[i] <- "Discipline Analysis"
      } else if (grepl("Moderation", all_significant_results$Analysis[i])) {
