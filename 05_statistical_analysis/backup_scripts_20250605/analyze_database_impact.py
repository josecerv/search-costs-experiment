...
class DatabaseImpactAnalyzer:
    ...
    def analyze_database_impact(self):
        ...
        for _, dept in dept_info.iterrows():
            ...
            for demo_group in ['female', 'URM', 'Black', 'Hispanic']:
                ...
                if demo_group == 'female':
                    ...
                    peer_speakers_demo = peer_speakers[peer_speakers['combined_race'] == 'hispanic']
        ...
        # Save results
        results_df.to_csv(self.outputs_dir / "database_impact_by_department.csv", index=False)
        summary_df.to_csv(self.outputs_dir / "database_impact_summary.csv", index=False)
        effects_df.to_csv(self.outputs_dir / "database_impact_treatment_effects.csv", index=False)
        
        # Create final report
        self.create_report(results_df, summary_df, effects_df)
        
        logger.info("Analysis complete!")
        
    def create_report(self, results_df: pd.DataFrame, summary_df: pd.DataFrame, effects_df: pd.DataFrame):
        """Create a comprehensive report of the analysis"""
        
        report = []
        report.append("# Database Impact Analysis Report\n")
        report.append(f"Generated: {pd.Timestamp.now()}\n")
        
        report.append("\n## Summary\n")
        report.append("This analysis examines whether departments selected speakers from the URM databases ")
        report.append("provided in the treatment condition, properly accounting for the peer university algorithm.\n")
        
        report.append("\n## Key Findings\n")
        
        for _, effect in effects_df.iterrows():
            demo = effect['demographic']
            report.append(f"\n### {demo.capitalize()} Speakers\n")
            report.append(f"- Treatment departments: {effect['treatment_mean_pct']:.2f}% from database\n")
            report.append(f"- Control departments: {effect['control_mean_pct']:.2f}% from database\n")
            report.append(f"- Effect size: {effect['effect_size_pct']:.2f} percentage points\n")
            report.append(f"- Relative increase: {effect['relative_increase_pct']:.1f}%\n")
            report.append(f"- Statistical significance: p = {effect['p_value']:.3f} ")
            report.append(f"({'significant' if effect['significant'] else 'not significant'})\n")
            
        report.append("\n## Detailed Statistics\n")
        
        for demo in ['female', 'URM', 'Black', 'Hispanic']:
            report.append(f"\n### {demo.capitalize()} Speakers\n")
            demo_summary = summary_df[summary_df['demographic'] == demo]
            
            for _, row in demo_summary.iterrows():
                report.append(f"\n#### {row['condition'].capitalize()} Condition\n")
                report.append(f"- Departments analyzed: {row['n_departments']}\n")
                report.append(f"- Total {demo} speakers: {row['total_demo_speakers']}\n")
                report.append(f"- Speakers from database: {row['total_matched_speakers']}\n")
                report.append(f"- Mean % from database: {row['mean_pct_from_database']:.2f}% (SD: {row['std_pct_from_database']:.2f})\n")
                report.append(f"- % of departments with any match: {row['pct_depts_with_any_match']:.1f}%\n")
                report.append(f"- Mean # in database shown: {row['mean_demo_in_database']:.1f}\n")
                
        report.append("\n## Methodology Notes\n")
        report.append("- Peer universities determined using ±20 rank algorithm (expanding to ensure 40 peers)\n")
        report.append("- Only speakers from peer universities included in analysis\n")
        report.append("- Matching based on LLM (GPT-4o) results accounting for name variations\n")
        report.append("- Treatment departments received tailored URM databases; control received generic links\n")
        
        # Write report
        with open(self.outputs_dir / "database_impact_analysis_report.md", "w") as f:
            f.writelines(report)
            
        # Also create a JSON summary for easy parsing
        json_summary = {
            "analysis_date": str(pd.Timestamp.now()),
            "treatment_effects": effects_df.to_dict(orient='records'),
            "summary_statistics": summary_df.to_dict(orient='records'),
            "key_findings": {
                demo: {
                    "treatment_mean": float(effects_df[effects_df['demographic'] == demo]['treatment_mean_pct'].values[0]),
                    "control_mean": float(effects_df[effects_df['demographic'] == demo]['control_mean_pct'].values[0]),
                    "effect_size": float(effects_df[effects_df['demographic'] == demo]['effect_size_pct'].values[0]),
                    "p_value": float(effects_df[effects_df['demographic'] == demo]['p_value'].values[0]),
                    "significant": bool(effects_df[effects_df['demographic'] == demo]['significant'].values[0])
                }
                for demo in ['female', 'URM', 'Black', 'Hispanic']
            }
        }
